---
title: 'Lake Turkana Isotope Hydrology: New measurements, evaporation model, and mass
  balance'
date: '2022-03-16'
output:
  html_notebook:
...
lake.height <- read.csv(here("Turkana-waters/data/GREALM_lake_height.csv")) |>
...
QGIS map is better — distinguish lake vs other samples 
...
Zoom time series to lake samples (except three anomalies) only 

```{r figure3, echo = FALSE, warning = FALSE}
fig3A <- ggplot() +
  
  geom_point(data = waters.meteoric |> filter(WaterType == "Lake"),
             aes(x = Date, y = d18O, color = WaterType, shape = WaterType), size = size.st, show.legend = FALSE) +

  labs(x = NULL,
       y = axis.x.label,
       title = "Meteoric waters") +
  
  scale_x_date(limits = as.Date(c("2016-01-01", "2021-12-31"))) +
  
  # Legend
  scale_shape_manual(
    breaks = c("Lake", "Restricted Lake", "River", "Precipitation", "Surface"), 
    # Set plot element shapes
    values = c(
      "Lake" = shape.lake,
      "Restricted Lake" = shape.surface.lake,
