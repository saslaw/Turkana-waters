...
pacman::p_load(here, knitr, tidyverse, readxl, lubridate, ggthemes, ggrepel, kableExtra)
```
...
LEL4 <- model.LEL(c(
  ...
  "dDp" = 10, 
  "k" = 0.1)) |>
  mutate(Scenario = "65% rh")
vary.rh <- rbind(LEL1, LEL2, LEL3, LEL4)
...
LEL4 <- model.LEL(c(
  ...
  "k" = 0.7)) |>
  mutate(Scenario = "0.7")
vary.k <- rbind(LEL1, LEL2, LEL3, LEL4)

# New for loop method ----
TC_v = c(25, 30, 35, 40)
k_v = c(0.1,0.3,0.5,0.7)
rh_v = c(0.2,0.35,0.5,0.65)
...
for (nn in 1:4){
  this.LEL = model.LEL(c(
    ...
    "dDp" = 10,
    "k" = k_v[nn])) |>
    mutate(Scenario = as.character(k_v[nn]))
  var_k <- rbind(var_k,this.LEL)
}
