...
pacman::p_load(here, knitr, tidyverse, lubridate, readxl, kableExtra, ggthemes)
```
...
*To do*: Create a table of input values and generate model results across multiple scenarios
...
```{r waters-all, echo = TRUE, message = FALSE}
...
### Load and process published data (@levin2009)

```{r waters-Levin, echo = TRUE, message = FALSE}
...
# Compare with Levin et al 2009 in lower alpha 
...
plot.all <- ggplot(data = waters.all, aes(x = d18O, y = dD, shape = WaterType, color = WaterType), show.legend = FALSE) +
  geom_point() +
  geom_point(data = waters.new, aes(x = "Calibrated d18O Mean", y = "Calibrated d2H Mean", shape = WaterType, color = WaterType), size = 3, show.legend = FALSE) +
  geom_abline(data = NULL, aes(slope = 8, intercept = 10, color = "GMWL")) +
